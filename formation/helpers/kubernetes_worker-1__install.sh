...
if [ ! -f $l_helpers_root/configurations/cks-worker-1_kubelet__kubeconfig.yaml ]; then
    echo "Missing worker-1 configuration file, exiting..."
    exit 1
fi

sudo cp $l_helpers_root/configurations/cks-worker-1_kubelet__kubeconfig.yaml /etc/kubernetes/kubelet.conf
sudo kubeadm join \
...
echo
echo
echo 'kubernetes worker-1 installed successfully'
